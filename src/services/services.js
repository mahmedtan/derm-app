...
export const getAllServices = async () => {
  const services = await client.fetch(
    '*[_type=="service" ]{showOnSlider,body,"image":image.asset->url,name,"slug":slug.current, serviceType->{name,"slug":slug.current},order,"sliderImages": sliderImages[].asset->url, showSlider, brands[]{"image":image.asset->url,brand,brandLink, products[]{"image":image.asset->url,productName, productPrice}}, metaTags }'
  );
  return services;
};
